...
exactly). Both flavors export the **same 41 symbols**; an acceptance gate
...
### Result deallocators (11) — call exactly one, matching the producer
...
`sparrow_engine_audio_event_result_free`,
...
| `sparrow_engine_detect_audio_events` | `sparrow_engine_audio_event_result_free` |
...
The 41-symbol set + signatures are a **stable contract**, evolved only by
