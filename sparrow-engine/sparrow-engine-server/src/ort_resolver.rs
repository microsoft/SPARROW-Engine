//! Self-contained ORT dylib discovery for the `sparrow-engine-server` binary.
//!
//! Mirrors `sparrow-engine-cli/src/ort_resolver.rs` (Phase D round 2 B-09 fix).
//! The server is built with `ort/load-dynamic` (same as the CLI + the Python
//! wheel per RP-3 / 2026-05-23), which means the `ort` crate `dlopen`s
...
//! own release tarballs don't ship the unversioned symlink, so an unaided
//! startup either dlopen-fails OR — as observed at Lane 5 (B-09) —
//! enters a long retry loop inside ORT's loader that LOOKS like a hang
//! because no log line fires (the tracing subscriber is initialized but
//! ORT's loader writes to its own internal logger before any Rust code
//! catches the failure).
//!
//! The Phase D R1 boot-trace instrumentation in `main.rs` localized the
//! hang to `Engine::new` (the marker `before engine_new` fires; the next
//! marker `engine created` never does). Pre-`Engine::new` invocation of
//! this shim removes the unset-ORT_DYLIB_PATH failure mode entirely.
//!
//! Layout expectation matches the CLI tarball (RP-4 Path B):
//!
//! ```text
//! sparrow-engine-server-<version>-<platform>/
//! ├── bin/sparrow-engine-server(.exe)            ← this binary
...
//!
//! Verbatim port of `sparrow-engine-cli/src/ort_resolver.rs` -- when
//! Phase F refactors this into `sparrow-engine-core` or a shared
//! `sparrow-engine-utils` crate, both copies disappear together.
