//! C FFI boundary layer for sparrow-engine-gpu.
//!
...
//! produce a `libsparrow_engine.so` cdylib with the same 41 `sparrow_engine_*` exported
...
//! surface duplicate intentionally. Phase C Acceptance Gate G5 asserts
//! the exported symbol set is identical on both flavors via `nm` diff.
//!
...
//!   - Allocator discipline: sparrow-engine-gpu allocates, sparrow-engine-gpu frees. Every `_new()` has `_free()`.
...
/// surface (41-symbol invariant enforced by the ABI acceptance gate).
///
/// # Safety
...
mod tests {
    use super::*;
    use crate::types::{AudioClass, AudioEvent, AudioSegment};
