//! CPU-flavor loading and inference for recording-level audio frame ensembles.
...
impl Engine {
    pub fn load_audio_ensemble(&self, path: impl AsRef<Path>) -> Result<AudioEnsembleHandle> {
        ...
        for member in &manifest.members {
            ...
            let session = self.load_ensemble_session(&model_path)?;
    ...
    fn load_ensemble_session(&self, path: &Path) -> Result<Session> {
        let mut builder = self
            .inner
            .session_builder
            .lock()
            .map_err(|_| SparrowEngineError::Ort("session_builder lock poisoned".to_string()))?
            .clone()
...
fn run_session(
    ...
    let input_value = TensorRef::from_array_view(view).map_err(crate::engine::ort_err)?;
