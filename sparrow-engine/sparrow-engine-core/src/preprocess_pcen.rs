...
impl PcenFrontend {
    ...
    pub fn stft_crop(&self, samples: &[f32]) -> Result<(Vec<f32>, usize, usize)> {
        ...
        for column in 0..columns {
            let frame_start =
                (column * self.config.hop_length) as i64 - center;
            for (index, value) in fft_input.iter_mut().enumerate() {
                let sample_index =
                    reflect_index(frame_start + index as i64, samples.len())?;
...
pub fn scipy_resample_poly(
    ...
    while upfirdn_output_len(
        filter_len + pre_pad + post_pad,
        samples.len(),
        up,
        down,
    )? < n_out + pre_remove
...
pub fn spectral_mean_subtraction(
    input: &[f32],
    rows: usize,
    columns: usize,
) -> Result<Vec<f32>> {
...
pub fn resize_bilinear_half_pixel(
    ...
    for output_y in 0..output_rows {
        let source_y = ((output_y as f32 + 0.5) * y_scale - 0.5)
            .clamp(0.0, (input_rows - 1) as f32);
        let y0 = source_y.floor() as usize;
        let y1 = (y0 + 1).min(input_rows - 1);
        let y_weight = source_y - y0 as f32;
        for output_x in 0..output_columns {
            let source_x = ((output_x as f32 + 0.5) * x_scale - 0.5)
                .clamp(0.0, (input_columns - 1) as f32);
...
fn frequency_bin(frequency: f32, n_fft: usize, sample_rate: u32) -> Result<usize> {
    let index =
        (f64::from(frequency) * 2.0 / sample_rate as f64 * (n_fft / 2 + 1) as f64).floor();
...
fn firwin_kaiser(length: usize, cutoff: f64, beta: f64) -> Vec<f32> {
    ...
    for index in 0..length {
        ...
        let window =
            modified_bessel_i0(beta * (1.0 - ratio * ratio).max(0.0).sqrt()) / denominator;
...
mod tests {
    ...
    fn scipy_polyphase_identity_is_exact() {
        ...
        assert_eq!(scipy_resample_poly(&input, 256_000, 256_000).unwrap(), input);
    }
    ...
    fn half_pixel_resize_matches_known_grid() {
        let output =
            resize_bilinear_half_pixel(&[1.0, 2.0, 3.0, 4.0], 2, 2, 1, 1).unwrap();
